            ...
            <div className="pb2">
              ...
              {product.avito_published && (
                <div style={{fontSize:11,color:"var(--muted)",marginTop:8}}>
                  Фид: <code style={{fontSize:10,cursor:"pointer",color:"var(--accent)"}} onClick={()=>copyText(location.origin+"/api/feeds/avito/"+product.store_id+".xml")} title="Нажмите, чтобы скопировать">{location.origin}/api/feeds/avito/{product.store_id}.xml</code>
                </div>
              )}
            </div>
          </div>
          ...
          {openSections.has("integrations") && <div className="pb2">
            ...
            <div style={{marginBottom:20}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:"var(--bg3)",borderRadius:8,border:"1px solid var(--border)"}}>
                ...
                <code style={{fontSize:11,color:"var(--accent)",flex:1,wordBreak:"break-all",cursor:"pointer"}} onClick={()=>copyText(location.origin+"/api/feeds/avito/"+current.id+".xml")} title="Нажмите, чтобы скопировать">{location.origin}/api/feeds/avito/{current.id}.xml</code>
                <a href={"/api/feeds/avito/"+current.id+".xml"} target="_blank" rel="noopener" className="btn btn-sm btn-outline" style={{flexShrink:0}}>Открыть</a>
              </div>
            </div>
            )}
  ...
  return (
    <>
      ...
      {Object.entries(byStore).map(([store, prods]) => {
        ...
        const feedUrl = si ? `/api/feeds/avito/${si.id}.xml` : null;
        return (
